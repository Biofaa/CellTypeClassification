...
filename_tmp=path_root/'data'/'HI_features.dat'
df_tmp=load_dat(filename_tmp)
...
x['glucose']=df['glucose']

x_tmp,y_tmp=Get_XY_FromDataFrame(df_tmp, 'cell_type')
x_tmp=x_tmp.iloc[:,3:]
x_tmp=x_tmp.drop(labels='sex', axis=1)
x_tmp['glucose']=df_tmp['glucose']
...
# %% compute score
print('XGBoost')
print('\ntest of excluded values')
score=performance_scores(model_xgb, x, y, compact=compact_score)
print(score)
